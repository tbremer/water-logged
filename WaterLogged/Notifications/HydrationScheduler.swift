...
///
/// One repeating calendar notification per active slot (e.g. 8:00, 9:00 … 22:00
/// for an hourly cadence) — ~15 pending requests for a typical window, well
/// under the system's 64-request limit.
///
/// Rebuilds come from a few places (launch, scene activation, settings changes,
/// the daily background refresh). Since a rebuild clears all pending requests
/// and then re-adds them across several `await`s, we keep a single in-flight
/// rebuild: a new request cancels the previous one and waits for it to unwind
/// before starting, so the clear/add steps never interleave. The whole type is
/// `@MainActor`, so it reads `AppSettings` directly (the `await`s are
/// non-blocking I/O, not work that needs to leave the main actor).
@MainActor
final class HydrationScheduler {
    ...
    /// The most recent (possibly still-running) scheduling task. A new request
    /// cancels it and waits for it to finish, so rebuilds never interleave.
    ...
    /// Rebuild the pending reminder set. Fire-and-forget; a new call supersedes
    /// any in-flight rebuild, so overlapping triggers collapse to the latest
    /// settings.
    func reschedule(settings: AppSettings) {
        let previous = rescheduleTask
        previous?.cancel()
        rescheduleTask = Task {
            _ = await previous?.value   // let a superseded run unwind first
            await self.performReschedule(settings: settings)
        }
    }

    /// Awaitable variant for the background-refresh task, which must stay awake
    /// until the rebuild has actually finished.
    func rescheduleNow(settings: AppSettings) async {
        reschedule(settings: settings)
        await rescheduleTask?.value
    }

    /// Rebuild the pending reminder set from current settings.
    private func performReschedule(settings: AppSettings) async {
